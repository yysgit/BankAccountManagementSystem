<template>
  <div>
    <div style="min-width:1000px">
      ...
      <Card shadow>
        <!--添加用户-->
        <Button v-if="buttonVerifAuthention('sys:admin:addAdminUser')" type="primary" icon="md-add" @click="addClick"
          style="margin-bottom: 10px;margin-right: 10px;">添加用户
        </Button>

        <!--显示全部用户-->
        <Button v-if="buttonVerifAuthention('sys:role:allList') && level!=2" type="primary"
          @click="queryAdminUserAllClick" style="margin-bottom: 10px;margin-right: 10px;">显示全部用户
        </Button>
        <!--菜单表格-->
        ...
        <!--添加用户-->
        <Modal v-model="modalAddAdminUser" title="添加用户" :mask-closable="false">
          ...
          <Form ref="formValidateAdd" :model="formValidateAdd" :rules="ruleValidateAdd" :label-width="80">
            ...
            <FormItem label="角色" prop="roleId">
              <Select v-model="formValidateAdd.roleId">
                <Option v-for="(item, index) in roleAllList" :key="index" v-text="item.roleName" :value="item.id+''">
                  {{item.roleName}}
                </Option>
              </Select>
            </FormItem>
        ...
        <!--编辑用户-->
        <Modal v-model="modalEditAdminUser" title="编辑用户" :mask-closable="false">
    ...
    data() {
      ...
      return {
        ...
        columns: [
          ...
          //   }
          // },
          { title: '员工id',  align: "center", key: 'id' },
          { title: '员工姓名',  align: "center", key: 'username'},
          { title: '登陆密码',  align: "center", key: 'password'},
          { title: '性别',  align: "center", key: 'sex'},
